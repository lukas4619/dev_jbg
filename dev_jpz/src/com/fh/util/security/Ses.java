...
public class Ses
{
  ...
  public Ses(String key)
    throws Exception
  {
    int len = key.length(); if (len < 16) throw new Exception("key length must be greater than or equal to 16 characters."); System.arraycopy(key.getBytes(Charset.forName("GB2312")), 0, this.key, 0, 16); for (int i = 0; i < 16; ) { int tmp161_160 = i;
      byte[] tmp161_157 = this.key; tmp161_157[tmp161_160] = (byte)(tmp161_157[tmp161_160] ^ this.SES_IV[i]); ++i;
    }
  }
  ...
  private void xor(byte[] output, int len) {
    ...
    for (int i = 0; i < len; ++i) {
      int tmp11_9 = i;
      byte[] tmp11_8 = output; tmp11_8[tmp11_9] = (byte)(tmp11_8[tmp11_9] ^ this.key[(key_position++)]);
