...
/**
 * 说明：我的预订记录 创建人：Lukas 18923798379 创建时间：2016-08-18
 */
@Controller
@RequestMapping(value = "/myReservation")
public class myReservationController extends BaseController {
	...
	public ModelAndView index() throws Exception{
		...
		mv.setViewName("front/member/reservation_list");
		return mv;
	}
	...
	public Object listPage() throws Exception {
		...
		pd.put("memberId", getWeChatOpenId());//当前用户标识
		pd.put("keywords", keywords);//
		pd.put("reservationType", -1);//
		pd.put("reservationStateID", -1);//
		...
		page.setPd(pd);
		varList = reservationService.list(page);	//查询当前用户所有预订记录
		map.put("type", 1);
		map.put("msg", "成功！");
		map.put("data", varList);
	 ...
	 /**去详情页面
		...
		public ModelAndView goDetails()throws Exception{
			...
			pd = reservationService.findByViewId(pd);	//根据ID读取
			mv.setViewName("front/member/reservation_detail");
