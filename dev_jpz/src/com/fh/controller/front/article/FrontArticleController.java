package com.fh.controller.front.article;
 ...
 */
@Controller
@RequestMapping(value="/frontArticle")
public class FrontArticleController extends BaseController {
	
	String menuUrl = "frontArticle/list.do"; //菜单地址(权限用)
...
//		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		...
		pd.put("CONTENT_SOURCE_URL", getRequertUrl()+"news/index.do?id="+ARTICLEID);
		pd.put("ARTICLEURL", getRequertUrl()+"news/index.do?id="+ARTICLEID);
		pd.put("EDITDATE", new Date());
		pd.put("ARTICLESTATEID", 17);// 添加数据是审核中的状态
		articleService.authorSave(pd);
		mv.addObject("msg","新增成功");
		mv.addObject("url","frontArticle/list.do");
	...
	public ModelAndView edit() throws Exception{
		...
		pd.put("CONTENT", pd.get("editorValue")+"");
		pd.put("EDITDATE", new Date());
		pd.put("CONTENT_SOURCE_URL", getRequertUrl()+"news/index.do?id="+pd.get("ARTICLEID")+"");
		pd.put("ARTICLEURL", getRequertUrl()+"news/index.do?id="+pd.get("ARTICLEID")+"");
	...
	public ModelAndView goEditAudit() throws Exception{
		...
		mv.setViewName("front/author/articleaudit_edit");
	...
	public ModelAndView editAudit() throws Exception{
		...
		articleauditService.save(pd);
		PageData p = new PageData();
		p.put("ARTICLEID", pd.getString("ARTICLEID"));
		p.put("ARTICLESTATEID", pd.getString("ARTICLESTATEID"));
		p.put("EDITDATE",new Date());
		articleService.editState(p);
	...
	public ModelAndView list(Page page) throws Exception{
		...
		if(member!=null){
			pd.put("authorId", member.getMemberId());
		}
		pd.put("articleTypeID", Convert.strToInt(pd.get("articleTypeID")+"", -1));
		pd.put("articleStateID", Convert.strToInt(pd.get("articleStateID")+"", -1));
		page.setPd(pd);
		...
		stateList = statesService.listAll(pd);
		mv.addObject("stateList", stateList);
		mv.setViewName("front/author/article_list");
...
//		mv.addObject("QX",Jurisdiction.getHC());	//按钮权限
	...
	public ModelAndView goAdd()throws Exception{
		...
		mv.setViewName("front/author/article_edit");
	...
	public Object pushArticle(){
		...
		try {
			...
			/**
			 * 功能描述:该处将软文封面图片上传到微信服务器采用的是临时图片素材，所以多次进行推送时需要进行校验，校验临时素材是否过期（3天有效期），避免重复调用上传临时图片素材接口逻辑
			 */
			if(Tools.isEmpty(THUMB_MEDIA_ID) || Tools.isEmpty(THUMB_CREATED_AT) || DateUtil.getDaySub(THUMB_CREATED_AT)>3){
				//图文封面上传情况
				String filePath = getPath()+"/uploadFiles/uploadImgs/"+pd.getString("ARTICLEIMG");
				...
				if (j.get("media_id") == null) {
					logger.info("上传图文图片素材到微信服务器异常，错误代码(errcode):"+j.get("errcode"));
					logger.info("上传图文图片素材到微信服务器异常，错误消息(errmsg):"+j.get("errmsg"));
					map.put("type", -1);
			...
			//验证当前软文是否上传微信服务器
			/**
			 * 功能描述:该处将软文上传到微信服务器采用的是临时图文素材，所以多次进行推送时需要进行校验，校验临时素材是否过期（3天有效期），避免重复调用上传临时图文素材接口逻辑
			 */
			if(Tools.isEmpty(MEDIA_ID) || Tools.isEmpty(MEDIA_ID_CREATED_AT) || DateUtil.getDaySub(MEDIA_ID_CREATED_AT)>3){
				// 调用微信接口上传图文并返回图文标识
				String resultStr = GroupMessage.uploadNews(messageMaterias);
				JSONObject j = JSONObject.fromObject(resultStr);
				if (j.get("media_id") == null) {
					logger.info("上传单图文到微信服务器异常，错误代码(errcode):"+j.get("errcode"));
			...
			pd.put("PUSHNUMBER", Convert.strToInt(pd.get("PUSHNUMBER")+"", 0)+1);//更新推送次数
			...
			if(openList.size()>0){
				for (int i = 0; i < openList.size(); i++) {
					...
					if(openList.get(i).get("groupId")!=null){
						//调用微信图文群发接口
						/**
						 * 描述：调用图文群发接口，如果关注用户数量太多（最多10000），那么则调用按关注用户分组进行发送，假设没有分组用户则需要用队列进行调用
						 */
						groupId = openList.get(i).get("groupId")+"";
						String resultJosin =GroupMessage.sendNewsMessage(groupId, media_id);//调用按分组推送图文
						JSONObject jsoin = JSONObject.fromObject(resultJosin);
						if (jsoin.get("errcode") == null || !jsoin.get("errcode").toString().equals("0")) {
							logger.info("推送单图文异常，错误代码(errcode):"+jsoin.get("errcode"));
							logger.info("推送单图文异常，错误消息(errmsg):"+jsoin.get("errmsg"));
						}
					}
				}
			}
	...
	public Object pushMPArticle(){
		...
		try {
			...
			for(int i=0;i<pdList.size();i++){
				...
				String THUMB_MEDIA_ID =  pdList.get(i).get("THUMB_MEDIA_ID")+"";	//当前软文封面微信标识
				String THUMB_CREATED_AT = pdList.get(i).get("THUMB_CREATED_AT")+"";//当前软文封面上传微信素材库返回时间戳
				...
				m.setContent_source_url(pdList.get(i).get("CONTENT_SOURCE_URL")+"");//设置图文信息详细页面阅读原文地址
				/**
				 * 功能描述:该处将软文封面图片上传到微信服务器采用的是临时图片素材，所以多次进行推送时需要进行校验，校验临时素材是否过期（3天有效期），避免重复调用上传临时图片素材接口逻辑
				 */
				if(Tools.isEmpty(THUMB_MEDIA_ID) || Tools.isEmpty(THUMB_CREATED_AT) || DateUtil.getDaySub(THUMB_CREATED_AT)>3){
					//图文封面上传情况
					String filePath = getPath()+"/uploadFiles/uploadImgs/"+pdList.get(i).get("ARTICLEIMG")+"";
					logger.info("图片物理路径："+filePath);
					String res = GongZhongService.uploadMedia("image", filePath);// 调用微信上传图片并返回图片标识
					JSONObject j = JSONObject.fromObject(res);
					if (j.get("media_id") == null) {
						logger.info("上传多文图片素材到微信服务器异常，错误代码(errcode):"+j.get("errcode"));
						logger.info("上传多文图片素材到微信服务器异常，错误消息(errmsg):"+j.get("errmsg"));
						map.put("type", -1);
		...
		try {
			...
			for(int i=0;i<pdList.size();i++){
				...
				if(Tools.isEmpty(THUMB_MEDIA_ID) || Tools.isEmpty(THUMB_CREATED_AT) || DateUtil.getDaySub(THUMB_CREATED_AT)>3){
					...
					JSONObject j = JSONObject.fromObject(res);
					if (j.get("media_id") == null) {
						logger.info("上传预览图文图片素材到微信服务器异常，错误代码(errcode):"+j.get("errcode"));
						logger.info("上传预览图文图片素材到微信服务器异常，错误消息(errmsg):"+j.get("errmsg"));
						map.put("type", -1);
			...
			media_id = j.get("media_id").toString();//微信接口上传图文并返回图文标识
			//调用微信图文预览接口
			String resultJosin =GroupMessage.sendNewsPreViewMessage(openId, media_id);
			JSONObject jsoin = JSONObject.fromObject(resultJosin);
			if (jsoin.get("errcode") == null || !jsoin.get("errcode").toString().equals("0")) {
