...
/**
 * 说明：门店 创建人：Lukas 18923798379 创建时间：2017-08-19
 */
@Controller
...
public class StoreController extends BaseController {
	...
	public ModelAndView save() throws Exception {
		...
		String encoderImgId = pd.get("ID") + ".png"; // encoderImgId此处二维码的图片名
		String filePath = PathUtil.getClasspath()+ Const.FILEPATHTWODIMENSIONCODE + encoderImgId; // 存放路径
		String url = getRequertUrl();
		if (url.contains("localhost")) {
			url = "http://lukas.jiabiango.com/";
		}
		String encoderContent = url + "frontStore/index.html?STOREID="
				+ pd.get("ID") + "";
		TwoDimensionCode.encoderQRCode(encoderContent, filePath, "png"); // 执行生成二维码
		pd.put("ENCODER", Const.FILEPATHTWODIMENSIONCODE + encoderImgId);
	...
	public ModelAndView edit() throws Exception {
		...
		pd = this.getPageData();
		pd.put("LASTDATE", new Date());
		if (StringUtil.isEmpty(pd.get("ENCODER"))) {
			String encoderImgId = pd.get("ID") + ".png"; // encoderImgId此处二维码的图片名
			...
			String encoderContent = url + "frontStore/index.html?STOREID="+ pd.get("ID") + "";
			TwoDimensionCode.encoderQRCode(encoderContent, filePath, "png"); // 执行生成二维码
			pd.put("ENCODER", Const.FILEPATHTWODIMENSIONCODE + encoderImgId);
		}
		storeService.edit(pd);
	...
	public ModelAndView list(Page page) throws Exception {
		...
		pd.put("storeStatusID",
				Convert.strToInt(pd.get("storeStatusID") + "", -1));
		page.setPd(pd);
