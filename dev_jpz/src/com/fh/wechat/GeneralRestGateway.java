...
{
  public static int handle(String key, int allowTimespanSeconds, HttpServletRequest request, HttpServletResponse response, GeneralRestGatewayInterface grgi, StringBuilder errorDescription)
    throws IOException
  {
    ...
    {
      throw new RuntimeException("在使用 com.fh.wechat.gateways.GeneralRestGateway.handle 方法解析参数并处理业务时，必须提供一个用于摘要签名用的 key (俗称 MD5 加盐)。");
    }
    ...
    for (Iterator localIterator = keys.iterator(); localIterator.hasNext(); ) { String t_key = (String)localIterator.next();
      if (t_key.equals("_s")) {
    ...
    throws UnsupportedEncodingException
  {
    ...
    {
      throw new RuntimeException("在使用 com.fh.wechat.gateways.GeneralRestGateway.buildUrl 方法构建通用 REST 接口 Url 时，不能使用 _s, _t 此保留字作为参数名。");
    }

    if (StringUtils.isBlank(key))
    {
      throw new RuntimeException("在使用 com.fh.wechat.gateways.GeneralRestGateway.buildUrl 方法构建通用 REST 接口 Url 时，必须提供一个用于摘要签名用的 key (俗称 MD5 加盐)。");
    }
