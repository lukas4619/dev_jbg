...
//启动聊天窗口并连接聊天服务器
 function creatw() {
			...
			function initWebSocket() {
				if (window.WebSocket) {
					...
					websocket.onopen = function() {
						//连接成功
						win.setTitle(title + '&nbsp;&nbsp;(已连接)   【现在全局对话】');
						websocket.send('be004977a6844865bc693c1193be8d4a'+user);
					}
			...
			win.on("close",function(){
				websocket.send('e128d0b8807b4a128d6390c44a6accef');
				isCreatw = false;
			 });
			...
			function send() {
				...
				if(toUser != ""){content = "e29149962e944589bb7da23ad18ddeed"+toUser+"eded77bdf35347249b2bacfa181c869d" + content;}
				var message = {};
				if (websocket != null) {
					if (input.getValue()) {
						Ext.apply(message, {
