...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="placeplu/list.do" method="post" name="Form" id="Form">
						<input type="hidden" name="SID" id="SID" value="${pd.SID}"/>
						<table style="margin-top:5px;">
							<tr>
												...
												<!-- 开始循环 -->
													<c:choose>
														<c:when test="${not empty storeList}">
															<c:forEach items="${storeList}" var="p" varStatus="vst">
																<option value="${p.ID }">${p.STORENAME }</option>
															</c:forEach>
														</c:when>
													</c:choose>
										</select>
								...
								<td style="vertical-align:top;padding-left:2px;">
								<select class="" name="shelvesId" id="shelvesId" data-placeholder="请选择"
											style="vertical-align:top;width: 120px;height: 32px;display: none;" >
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>placeplu/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>placeplu/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>placeplu/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>placeplu/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>placeplu/excel.do';
		}
		...
		function loadShelves(){
			...
			$.ajax({
				type: "POST",
				url: '<%=basePath%>placeplu/loadShelves.do?storeId='+storeId+'&tm='+new Date().getTime(),
				dataType:'json',
				//beforeSend: validateData,
				cache: false,
				success: function(data){
					if(data.msg=='ok'){
						var html='';
						 $.each(data.pdList, function(i, list){
							 html+='<option value="'+list.ID+'">'+list.SHELVESNAME+'</option>';
						 });
						 $("#shelvesId").html(html);
						 if($("#SID").val()!='' && $("#SID").val() !=-1){
							 $("#shelvesId").val($("#SID").val());
						 }
						 $("#shelvesId").show();
					}else{
						$("#shelvesId").hide();
