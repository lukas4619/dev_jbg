...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="types/list.do" method="post" name="Form" id="Form">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>
											<c:if test="${var.TYPECLASS==1}">
											用户
											</c:if>
											<c:if test="${var.TYPECLASS==2}">
											产品
											</c:if>
											<c:if test="${var.TYPECLASS==3}">
											预订
											</c:if>
											<c:if test="${var.TYPECLASS==4}">
											软文
											</c:if>
											<c:if test="${var.TYPECLASS==5}">
											收益
											</c:if>
											<c:if test="${var.TYPECLASS==6}">
											提现
											</c:if>
											<c:if test="${var.TYPECLASS==7}">
											图片
											</c:if>
											<c:if test="${var.TYPECLASS==8}">
											微信消息
											</c:if>
											<c:if test="${var.TYPECLASS==9}">
											优惠券类型
											</c:if>
											</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>types/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 250;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>types/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>types/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 250;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>types/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>types/excel.do';
		}
	</script>
