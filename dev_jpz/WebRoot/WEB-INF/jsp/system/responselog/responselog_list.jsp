...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="responselog/list.do" method="post" name="Form" id="Form">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">接收消息类型</th>
									<th class="center">发送消息类型</th>
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input  <c:if test="${var.id <10 }">disabled="disabled" </c:if>type='checkbox' name='ids' value="${var.id}" class="ace" /><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class='center'>${var.typeName}</td>
											<td class='center'>
											<c:choose>
											<c:when test="${var.sendType==1}">
											文本
											</c:when>
											<c:otherwise>
											图文
											...
											<td class='center'>
											<label>
        							      <input name="switch-field-1" onclick="editByIsValId('${var.id}','${var.isValid}');" class="ace ace-switch ace-switch-3" <c:if test="${var.isValid==1}"> checked="checked" </c:if>  type="checkbox">	<span class="lbl"></span>
    										</label>
											</td>
											...
											<td class='center'>${var.adminId}</td>
											<td class="center">
	...
	<script type="text/javascript">
		...
		function add(){
			 top.jzts();
			 window.location.href="<%=basePath%>responselog/goAdd.do";
		}
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>responselog/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		//修改状态
		function editByIsValId(Id,IsValId){
			if(IsValId==1){
				IsValId=2
			}else{
				IsValId=1;
			}
			var url = "<%=basePath%>responselog/editByIsValId.do?ID="+Id+"&ISVALID="+IsValId+"&tm="+new Date().getTime();
			$.get(url,function(data){
				//nextPage(${page.currentPage});
			});
		}
		...
		function edit(Id){
			 top.jzts();
			 window.location.href="<%=basePath%>responselog/goEdit.do?ID="+Id;
		}
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>responselog/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>responselog/excel.do';
		}
	</script>
