...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="place/list.do" method="post" name="Form" id="Form">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">架位名称</th>
									<th class="center">架位状态</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.PLACETYPENAME}</td>
											<td class='center'>${var.CREATEDATE}</td>
											<td class='center'>${var.LASTDATE}</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-light btn-xs" onclick="goUploadExcel(${var.ID},${var.STOREID },${var.SHELVESID });" title="批量导入选择商品">
													<i id="nav-search-icon" class="ace-icon fa fa-cloud-upload bigger-110 nav-search-icon blue"></i>
													</a>
													<a class="btn btn-xs btn-info" title="选择货位商品" onclick="userBinding(${var.ID},${var.STOREID },${var.SHELVESID });">
	...
	<script type="text/javascript">
		...
		function add(){
			 top.jzts();
			// window.location.href="<%=basePath%>place/goAdd.do";
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>place/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>place/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 top.jzts();
			// window.location.href="<%=basePath%>place/goEdit.do?ID="+Id;
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>place/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>place/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>place/excel.do';
		}
		
		//打开上传页面
		function goUploadExcel(placeId,storeId,shelvesId){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="EXCEL 导入到数据库";
			 diag.URL = '<%=basePath%>place/goUploadExcel.do?storeId='+storeId+'&shelvesId='+shelvesId+'&placeId='+placeId;
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		//打开绑定用户窗口
		function userBinding(placeId,storeId,shelvesId){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="选择货位商品";
			 diag.URL = '<%=basePath%>place/pluList.do?storeId='+storeId+'&shelvesId='+shelvesId+'&placeId='+placeId;
			 diag.Width = 900;
			 diag.Height = 500;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
			 ...
			 diag.CancelEvent = function(){ //关闭事件
				if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
					var pluCodes = diag.innerFrame.contentWindow.document.getElementById('USERNAME').value;
					var url = "<%=basePath%>placeplu/savePlu.do?pluCodes="+pluCodes+"&shelvesId="+shelvesId+"&placeId="+placeId+"&storeId="+storeId+"&tm="+new Date().getTime();
					$.get(url,function(data){
						$("#fhadmincz").tips({
							side:1,
				            msg:'绑定成功',
