...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="inventory/list.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;">
								 	排序：<select class="chosen-select form-control" data="${pd.sortsFied}" name="sortsFied" id="sortsFied" data-placeholder="请选择" style="vertical-align:top;width:40px;">
												<option value="inventoryCount">库存数量</option>
												<option value="taps">预警数量</option>
										</select>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center' style="font-weight: bold;color: red;">
											${var.INVENTORYCOUNT}
											<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="核准库存" onclick="add('${var.TINVENTORYID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="修改库存"></i>
													</a>
													</c:if>
											</td>
											<td class='center' style="font-weight: bold;color:red;">${var.TAPS}<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑预警值" onclick="edit('${var.TINVENTORYID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="修改预警值"></i>
													</a>
													</c:if></td>
										</tr>
	...
	<script type="text/javascript">
		...
		function add(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="核准库存";
			 diag.URL = '<%=basePath%>inventory/goAdd.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 300;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>inventory/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑预警值";
			 diag.URL = '<%=basePath%>inventory/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 150;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>inventory/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>inventory/excel.do';
		}
	</script>
