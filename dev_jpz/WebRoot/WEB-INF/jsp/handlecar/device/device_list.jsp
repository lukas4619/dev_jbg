...
<body class="no-skin">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.DEVICENAME}</td>
											<td class='center'>${var.ATTACHNAME}</td>
											<td class='center'>${var.DEVICEDEPICT}</td>
											...
											<td class='center'>
											<fmt:formatDate value="${var.UPDATEDATE}" pattern="yyyy-MM-dd HH:mm:ss"  />
											</td>
											<td class="center">
	...
	<script type="text/javascript">
		...
		function toExcel(){
			...
			var deviceAttach =$("#deviceAttach").val();
			var deviceStatus =$("#deviceStatus").val();
			var lastStart =$("#lastStart").val();
			var lastEnd =$("#lastEnd").val();
			window.location.href="<%=basePath%>device/excel.do?keywords="+keywords+"&deviceType="+deviceType+"&deviceAttach="+deviceAttach+"&deviceStatus="+deviceStatus+"&lastStart="+lastStart+"&lastEnd="+lastEnd;
		}
		//打开上传excel页面
		function fromExcel(){
			 top.jzts();
			 ...
			 diag.URL = '<%=basePath%>device/goUploadExcel.do';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
