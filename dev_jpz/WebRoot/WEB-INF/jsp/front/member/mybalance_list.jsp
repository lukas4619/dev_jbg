...
<div class="nolist txtcenter">
    ...
    <div class="haslist">
        <div class="detailsTitle">我的提现</div>
    	<header>
	    	<div class="width1">提现时间</div>
	    	<div class="width2">提现金额</div>
	    	<div class="width3">提现类型</div>
    	</header>
    	<div class="lists">
    		<ul>
    		</ul>
...
function listPage(currentPage){
	...
	$.post("<%=basePath%>myBalance/listPage.do",param,function(data){
			var list = data.data;
			...
			for(var i=0;i<list.length;i++){
				pageHtml+='<a style="color: #777;" href="<%=basePath%>myBalance/goDetails?id='+list[i].balanceID+'">';
				pageHtml+='<li><span class="width1">'+(new Date(list[i].createDate)).format("yyyy-MM-dd hh:mm")+'</span>';
				pageHtml+='<span class="width2" style="color: #f60;font-weight: bold;">￥'+list[i].balanceMoney+'</span><span class="width3">';
				pageHtml+='<a class="blue" href="<%=basePath%>myBalance/goDetails?id='+list[i].balanceID+'">'+list[i].typeName+'</a></span></li>';
				pageHtml+='</a>';
			}
